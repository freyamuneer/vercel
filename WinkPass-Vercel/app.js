...
async function initMasterPassword() {
  try {
    ...
    if (response.ok) {
      ...
      if (logs.length) {
        logTerminal(`[BACKEND] Connected to WinkPass cryptographic vault. ${logs.length} audit event(s) loaded.`, 'green');
      } else {
        logTerminal('[BACKEND] Connected to WinkPass cryptographic vault.', 'green');
      }
    } else {
      throw new Error('API unavailable');
    }
  } catch (error) {
    logTerminal('[BACKEND_WARN] API connection unavailable. Local demo mode active.', 'yellow');
  }
}
...
async function verifyPassword() {
  ...
  logTerminal(`[AUTH_EXEC] Sending ocular token sequence to cryptographic vault...`, 'text-muted');

  try {
    const response = await fetch('/api/verify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sequence: state.authTokens })
    });

    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Verification failed');

    const inputHash = data.hash_digest || '';
    logTerminal(`[DIGEST] Server SHA-256: ${inputHash.slice(0, 16)}...`, 'text-muted');
    ...
    if (data.success) {
      ...
      logTerminal(`[AUTH_SUCCESS] 256-BIT OCULAR KEY MATCH CONFIRMED BY BACKEND.`, 'green');
    } else {
      ...
      resultMsg.textContent = data.message || 'Unauthorized ocular signature detected. Audit log recorded.';
      logTerminal(`[AUTH_FAIL] Backend cryptographic verification denied access.`, 'magenta');
    }
  } catch (error) {
    playSoundEffect('ACCESS_DENIED');
    resultBanner.className = 'result-banner error';
    resultIcon.textContent = '⚠️';
    resultTitle.textContent = 'VAULT OFFLINE // API ERROR';
    resultMsg.textContent = error.message || 'Unable to reach the cryptographic backend.';
    logTerminal(`[BACKEND_ERR] ${error.message}`, 'magenta');
  }
}
...
async function saveMasterKey() {
  ...
  try {
    logTerminal('[ENROLL_EXEC] Sending new ocular key to backend vault...', 'text-muted');
    const response = await fetch('/api/enroll', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sequence: state.enrollTokens })
    });

    const data = await response.json();
    if (!response.ok || !data.success) throw new Error(data.error || data.message || 'Enrollment failed');

    logTerminal(`[ENROLL_SUCCESS] Master ocular key updated on backend. Hash: ${(data.new_master_hash || '').slice(0, 24)}...`, 'green');
    alert(`Master Key Enrolled Successfully!\nSequence: ${state.enrollTokens.join(' -> ')}\nSHA-256 Hash stored in the WinkPass backend vault.`);
    tabAuth.click();
  } catch (error) {
    logTerminal(`[ENROLL_ERR] ${error.message}`, 'magenta');
